...
リポジトリ直下の `.env` にBotトークンを設定します。

```text
SCHEDTOOL_DISCORD_TOKEN=Discord Bot Token
```

VPS上でリポジトリ直下に移動して起動します。

```bash
docker compose --profile schedtool up -d schedtool-bot
```

ログ確認:

```bash
docker compose logs -f schedtool-bot
```

停止:

```bash
docker compose stop schedtool-bot
```
