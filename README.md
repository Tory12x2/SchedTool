...
現在の設定がEmbedで表示され、通知チャンネル、イベント名と日数、回答締切、開催日通知、メンション設定を画面上から変更できます。

Discordサーバー内に、日程調整用チャンネルと通知用チャンネルを分けて作っておくと運用しやすくなります。
日程調整用チャンネルでは `/schedule` や `/auto_schedule_start` を実行し、通知用チャンネルは `/setup` で設定します。
通知用チャンネルを未設定のまま使い始めた場合は、最初に日程調整を作成したチャンネルが通知先になります。

必要に応じて、日程調整の対象メンバー用ロールも作成し、`/setup` の「参加予定者ロール（任意）」で設定してください。
...
締切の初期値は、各日程の1日前0時です。実際にはその時刻以降に回答ボタンが無効化されます。

開始日が当日の範囲は例外として、当日 `23:59` まで回答できます。
締切を過ぎると、Botが自動で対象の日付範囲ボタンを無効化します。

締切のデフォルトを変えたい場合は、管理者が次を実行します。

```text
/deadline_setting days_before:1 hour:0
```

この設定は、今後作成する日程調整に反映されます。既存の日程調整の締切は変更しません。
...
設定系の個別コマンドも残っていますが、通常は `/setup` から変更するのがおすすめです。
管理者向けコマンドは、Discord上でも管理者権限のあるメンバーにだけ表示されるように設定しています。
